...
export function ProposalNegotiations({ proposalId, isProposalOwner }: ProposalNegotiationsProps) {
  const router = useRouter();
  const [showCompleted, setShowCompleted] = useState(false);
  const { conversations, completedConversations, isLoading } = useProposalNegotiations(proposalId);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h3 className="text-xl font-semibold">Negotiations</h3>
            ...
            <Card key={conversation.id} className="hover:shadow-md transition-shadow">
                  ...
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <CardDescription className="flex items-center gap-2">
                        ...
                        {isProposalOwner ? "Interested in your proposal" : "Discussion partner"}
                      </CardDescription>
                    </div>
                  </div>
              ...
              <CardContent>
                    ...
                    <div>
                      ...
                      <div className="font-medium">
                        {conversation.activeNegotiations[0]?.status && 
                          getNegotiationStatusBadge(conversation.activeNegotiations[0].status)
                        }
                      </div>
                    </div>
                  </div>
                ...
                {conversation.activeNegotiations.map((negotiation) => (
                  <div key={negotiation.proposalId} className="bg-muted/30 rounded-md p-3 mb-4">
                    <div className="flex items-center justify-between">
                ...
                <div className="flex items-center justify-between">
                  ...
                  {!showCompleted && (
                    <Button
                      onClick={() => {
                        const negotiation = conversation.activeNegotiations[0];
                        router.push(`/matches/${negotiation.matchId}/negotiate/${negotiation.proposalId}/${conversation.otherParty.id}`);
                      }}
                      className="ml-4"
                    >
