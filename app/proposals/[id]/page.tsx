                ...
                <CardContent className="pt-6">
                  <div className="space-y-6">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <div>
                            <p className="text-2xl font-bold text-green-900">
                              {(proposal.ndaSignatures ? proposal.ndaSignatures.length : 0) + approvedNdaRequests.length}
                            </p>
                            <p className="text-sm text-green-700">Users with Access</p>
                          </div>
                        </div>
                      </Card>
                      <Card className="p-4">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-blue-700">
                              ...
                              {(proposal.ndaSignatures ? proposal.ndaSignatures.length : 0) + approvedNdaRequests.length} users with access • 
                    ...
                    <div className="space-y-4">
                      ...
                      {(proposal.ndaSignatures && proposal.ndaSignatures.length > 0) || loadingNdaSignatures || loadingApprovedRequests || approvedNdaRequests.length > 0 || isProposalOwner ? (
                        <div>
                          <div className="flex items-center gap-2 mb-3">
                            <Badge variant="outline" className="text-xs">ACCESS GRANTED</Badge>
                            <span className="font-medium">
                              Users with NDA Access
                              {!loadingNdaSignatures && !loadingApprovedRequests && ` (${(proposal.ndaSignatures?.length || 0) + approvedNdaRequests.length + (isProposalOwner ? 1 : 0)})`}
                              {(loadingNdaSignatures || loadingApprovedRequests) && " (Loading...)"}
                            </span>
                          </div>
                          ...
                          ) : (proposal.ndaSignatures && proposal.ndaSignatures.length > 0) || approvedNdaRequests.length > 0 || isProposalOwner ? (
                            <div className="space-y-2">
                              ...
                              {isProposalOwner && (
                                <div className="flex items-center justify-between p-3 rounded-lg border bg-blue-50 border-blue-200">
                                  ...
                                  <Badge variant="secondary" className="text-xs bg-blue-200 text-blue-800">Owner</Badge>
                                </div>
                              )}
                              
                              {/* Show approved NDA requests */}
                              {approvedNdaRequests.map((request) => (
                                <div key={request.id} className="flex items-center justify-between p-3 rounded-lg border bg-green-50 border-green-200">
                                  <div className="flex items-center gap-3">
                              ...
                              {(!proposal.ndaSignatures || proposal.ndaSignatures.length === 0) && approvedNdaRequests.length === 0 && isProposalOwner && (
                                <div className="p-3 rounded-lg border bg-gray-50">
                                  <p className="text-sm text-muted-foreground">No additional users have NDA access yet</p>
                                </div>
                              )}
                            </div>
                          ) : (
                            <div className="p-3 rounded-lg border bg-gray-50">
                              <p className="text-sm text-muted-foreground">No users with NDA access yet</p>
                            </div>
                          )}
                        </div>
                      ) : null}
                    ...
                    {((proposal.ndaSignatures && proposal.ndaSignatures.length > 0) || approvedNdaRequests.length > 0) && (
                      <div>
                        <div className="flex items-center gap-2 mb-3">
                        ...
                        <div className="space-y-2">
                          ...
                          {approvedNdaRequests
                            ...
                            .map((request) => (
                            <div key={`approved-${request.id}`} className="p-3 rounded-lg border bg-green-50 border-green-200">
                              <div className="flex items-center gap-2 mb-2">
                                <CheckCircle className="h-4 w-4 text-green-600" />
                                <span className="text-sm font-medium text-green-900">
                                  NDA Access Approved for {request.signerName || `User ${request.signerUserId}`}
                                  {request.signerUserId === user.id && " (You)"}
                                </span>
                              </div>
                              <p className="text-xs text-green-700 ml-6">
                                {new Date(request.approvedAt).toLocaleDateString()} at {new Date(request.approvedAt).toLocaleTimeString()}
                              </p>
                            </div>
                          ))}
                          ...
                          {proposal.ndaSignatures && proposal.ndaSignatures.length > 0 && 
                            ...
                            .map((signature, index) => (
                            <div key={`signature-${signature.id}`} className="p-3 rounded-lg border bg-green-50 border-green-200">
                              <div className="flex items-center gap-2 mb-2">
                                <CheckCircle className="h-4 w-4 text-green-600" />
                                <span className="text-sm font-medium text-green-900">
